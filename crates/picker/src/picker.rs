...
pub struct Picker<D: PickerDelegate> {
    ...
    /// Whether a modal hosting this picker should be stashed (kept alive and
    /// hidden) when dismissed, so `workspace::ReopenLastModal` can reveal it
    /// again. Defaults to `true`; configure with [`Picker::reopenable`].
    reopenable: bool,
}
...
impl<D: PickerDelegate> Picker<D> {
    ...
    /// `workspace::ReopenLastModal` after it's dismissed. Defaults to `true`;
